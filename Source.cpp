...
int main()
{
	...
	int timer = 0, n=0, index;
	all_text text;
	all_text font;
	all_text hp; 
	all_text point;
	textInit(text, font);
	PText1(text, timer, hp, point);
	...
	while (window.isOpen())
	{
		...
		if (hp.hp != 0) {
			...
			if ((fence.sprite.getPosition().x + 100 >= car1.sprite.getPosition().x && car1.sprite.getPosition().x + 100 >= fence.sprite.getPosition().x)
				&& (fence.sprite.getPosition().y <= car1.sprite.getPosition().y + 200 && car1.sprite.getPosition().y <= fence.sprite.getPosition().y + 75))
			{
				hp.hp--;
				text.text[2].setString(std::to_string(hp.hp));
